...
#' Simulate from a linear state space model.
#'
#' \code{SimKFc} implements the simulation from a linear state space model.
#'
#' The function takes several arguments that specify the linear state space model
#' and returns a list consisting of the simulated latent process and the observations.
#'
...
#'
#' @export
SimKFc <- function(mX,Gam,Bet,mG,h0,mX0)
{
  iT=dim(mX)[1]
  iq=dim(Gam)[2]
  ip=dim(Gam)[1]-iq
  ...
  return( .Call(`_LinKF_mcalc_sim`,iT, ip, iq, t(mX), me, Gam, Bet, mG, h0, mX0) )
}
